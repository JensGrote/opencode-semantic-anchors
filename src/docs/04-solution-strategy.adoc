...
. *Take* – Does something already exist that we can use directly? (e.g., opencode Plugin SDK)
. *Buy* – Can we fork/integrate an open-source component? (e.g., `agentcontract/spec` terminology, Semantic-Anchors anchor IDs)
. *Make* – Custom development only for the opencode-specific part (plugin hooks, YAML config loader)
