...
. *`permission.ask`* — Wird vor jeder Tool-Ausführung aufgerufen. Kann die Ausführung erlauben oder blocken. Der User sieht einen Permission-Dialog.
. *`tool.execute.before`* — Wird ebenfalls vor jeder Tool-Ausführung aufgerufen. Kann `throw new Error()` zum Blocken nutzen, oder den Durchlauf erlauben.
