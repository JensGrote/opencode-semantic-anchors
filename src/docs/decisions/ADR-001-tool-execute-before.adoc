...
. *`permission.ask`* — Called before every tool execution. Can allow or block execution. The user sees a permission dialog.
. *`tool.execute.before`* — Also called before every tool execution. Can use `throw new Error()` to block, or allow the execution to proceed.
