...
. *Build from scratch* — Alles selbst schreiben, maximale Kontrolle
. *Take before Buy before Make* — Existierende Lösungen prüfen, übernehmen (Take), integrieren (Buy), oder erst dann Eigenentwicklung (Make)
...
. *Take* — Existiert etwas, das wir direkt nutzen können?
* opencode Plugin SDK (take: nutzen wir direkt)
* Semantic-Anchors Terminologie (take: übernehmen wir)
. *Buy* — Können wir eine OpenSource-Komponente integrieren?
...
. *Make* — Eigenentwicklung nur für den opencode-spezifischen Teil:
