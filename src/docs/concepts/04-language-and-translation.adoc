...
. *Domain Correctness* — Which translation most precisely represents the technical term in the context of the system architecture?
. *Intent Anchor* — What is supposed to happen per definition? The translation must convey the purpose of the concept.
. *Established Terminology* — Is there an established German technical term? (e.g., "Bausteinsicht" for "Building Block View" in arc42)
...
Every translation must be verifiable in the git diff. A German version never silently changes the content of the English version. If a translation _must_ deviate from the English (e.g., because a concept is structured differently in German), the deviation is marked with a comment:
